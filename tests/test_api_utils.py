...
class TestExtractK2HorizonMessages:
    """K2 Horizon's template needs a thinking field on every assistant turn."""

    def test_assistant_turn_without_reasoning_gets_empty_reasoning_content(self):
        ...
        result = extract_k2_horizon_messages(messages)
    ...
    def test_native_reasoning_and_tool_calls_are_preserved(self):
        from omlx.api.utils import extract_k2_horizon_messages
        ...
        result = extract_k2_horizon_messages(messages, tokenizer=_K2StubTokenizer())

        assistant = result[1]
        assert assistant["reasoning_content"] == "R"
    ...
    def test_already_normalized_dicts_pass_through_with_default(self):
        from omlx.api.utils import extract_k2_horizon_messages
        ...
        assert result[2]["reasoning_content"] == "R"
        assert messages[1] == {"role": "assistant", "content": "A"}

    def test_k2_horizon_uses_native_reasoning_content(self):
        assert uses_native_reasoning_content(config_model_type="k2_horizon") is True


class _K2StubTokenizer:
    """Tokenizer stub that reports native tool calling like the K2 wrapper."""
